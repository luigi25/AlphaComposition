#ifndef ALPHACOMPOSITION_PARALLEL_ALPHA_BLEND_H
#define ALPHACOMPOSITION_PARALLEL_ALPHA_BLEND_H
...
vector<double> parallelOpenMPTest(int n_threads, int img_size, float* flatten_comb_img, float* flatten_imgA, float* flatten_imgB){
    int num_executions = 100;
    vector<double> mean_executions_time_vec;
    for(int n_thread = 1; n_thread <= n_threads; n_thread++) {
        ...
        for (int execution = 0; execution < num_executions; execution++) {
            ...
            #pragma omp parallel num_threads(n_thread) default(none) shared(img_size, flatten_comb_img, flatten_imgA, flatten_imgB)
            {
...
//            }
        ...
        mean_executions_time_vec.push_back(mean_executions_time/num_executions);
    }

    return mean_executions_time_vec;
}

#endif //ALPHACOMPOSITION_PARALLEL_ALPHA_BLEND_H
